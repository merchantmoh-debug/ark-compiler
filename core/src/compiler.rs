...
use crate::ast::{ArkNode, Expression, Statement, FunctionDef};
...
impl fmt::Display for CompileError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "Compilation Error: {} at {}:{}:{}", self.message, self.file, self.line, self.column)
    }
}
...
fn fold_stmt(stmt: &Statement) -> Statement {
    match stmt {
        ...
        Statement::If { condition, then_block, else_block } => Statement::If {
            ...
            else_block: else_block.as_ref().map(|b| b.iter().map(fold_stmt).collect()),
        },
        ...
        Statement::SetField { obj_name, field, value } => Statement::SetField {
    ...
    match expr {
        Expression::Call { function_hash, args } => {
            ...
            if folded_args.iter().all(|a| matches!(a, Expression::Literal(_))) {
                let literals: Vec<&String> = folded_args.iter().map(|a| match a {
                ...
                match function_hash.as_str() {
                    "intrinsic_add" | "add" if literals.len() == 2 => {
                        if let (Ok(a), Ok(b)) = (literals[0].parse::<i64>(), literals[1].parse::<i64>()) {
                    ...
                    "intrinsic_div" | "div" if literals.len() == 2 => {
                        if let (Ok(a), Ok(b)) = (literals[0].parse::<i64>(), literals[1].parse::<i64>()) {
                            if b != 0 { return Expression::Literal((a / b).to_string()); }
                        }
                    }
                    ...
                    // Boolean logic
                     "intrinsic_and" | "and" => { // Can be variadic or binary? Usually binary.
                        ...
                        // "true" / "false"
                        let bools: Result<Vec<bool>, _> = literals.iter().map(|s| s.parse::<bool>()).collect();
            ...
            Expression::Call { function_hash: function_hash.clone(), args: folded_args }
        }
            ...
            match condition {
                ...
                Expression::Literal(s) if s == "false" => {
                    if let Some(else_stmts) = else_block {
                        let optimized_block: Vec<Statement> = else_stmts.iter().map(dce_stmt).collect();
                        Statement::Block(optimized_block)
                    } else {
                        Statement::Block(vec![])
                    }
                ...
                _ => {
                    ...
                    Statement::If {
                        ...
                        else_block: else_block.as_ref().map(|b| b.iter().map(dce_stmt).collect()),
                    }
                }
            }
        }
            ...
            Statement::Let { name, ty: _, value } => {
                ...
                if let Some(scope) = self.scopes.last() {
                    if scope.contains(name) {
                        println!("Compiler Warning: Variable '{}' shadows an existing variable in the current scope.", name);
                    }
                }
                ...
                match function_hash.as_str() {
                    "intrinsic_add" | "add" => {
                        if args.len() == 2 {
                            ...
                            return Err(CompileError { message: format!("add requires 2 args, got {}", args.len()), line: 0, column: 0, file: "unknown".into() });
                        }
                    }
                    "intrinsic_sub" | "sub" => {
                        if args.len() == 2 {
...
mod tests {
    ...
    fn test_dead_code_if_true() {
        let stmt = Statement::If {
            ...
            else_block: Some(vec![Statement::Return(Expression::Literal("2".to_string()))]),
        };
        let optimized = dce_stmt(&stmt);
        if let Statement::Block(stmts) = optimized {
            assert_eq!(stmts.len(), 1);
