...
mod tests {
    ...
    #[test]
    fn test_glenn_lewis_verification_generated_json() {
        // This test loads the JSON generated by meta/ark_to_json.py from tests/glenn_lewis.ark
        // It proves that the "Physics Engine" catches the resource leak.
        use crate::loader::load_ark_program;
        use std::fs;
        use std::path::PathBuf;

        let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        path.push("../tests/glenn_lewis.json");

        if !path.exists() {
            // Fallback for different CWD
            path = PathBuf::from("../../tests/glenn_lewis.json");
        }

        println!("Loading test file: {:?}", path);
        let json_content = fs::read_to_string(&path);

        if let Ok(content) = json_content {
            let mast = load_ark_program(&content).expect("Failed to load MAST");
            let result = LinearChecker::check(&mast.content);

            match result {
                Err(LinearError::UnusedResource(name)) => {
                    println!("Successfully caught UnusedResource: {}", name);
                    assert_eq!(name, "buf", "Expected 'buf' to be the unused resource");
                }
                Err(e) => panic!("Expected UnusedResource, got validation error: {:?}", e),
                Ok(_) => panic!(
                    "LINEAR CHECK FAILED: The compiler did NOT catch the resource leak! The Physics Engine is OFF."
                ),
            }
        } else {
            // If file missing, maybe skip or fail. For this specific task, we want to know.
            panic!("Could not find tests/glenn_lewis.json. Did you run meta/ark_to_json.py?");
        }
    }
